...
auto is_valid_password(const std::string& database_row) -> bool
{
    auto result = false;
...
auto main(int argc, char** argv) -> int
{
    ...
    auto valid_database_rows = std::vector<std::string>();
    std::for_each(corrupted_database.begin(), corrupted_database.end(), 
        [&](const auto& database_row) 
        { 
            if(is_valid_password(database_row)) 
            { 
                valid_database_rows.emplace_back(database_row); 
            }
        }
    );
    answer.first = valid_database_rows.size();
